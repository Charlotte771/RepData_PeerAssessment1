...
```{r total steps/day}
s_d <- aggregate(activity$steps, by = list(date=activity$date), FUN=sum)
colnames(s_d) <- c("Date", "Steps")
```

```{r histogram of total steps per day}
qplot(s_d$Steps, geom = "histogram", xlab = "Number of Steps/Day", ylab = "Frequency", main="Total Steps Per Day")
```
...
median(s_d$Steps)
```
**The mean steps taken each day is 10766 and the median steps taken each day is 10765.**
